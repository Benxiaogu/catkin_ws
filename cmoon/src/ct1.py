...
class Controller:
    ...
    def control(self, place):
        ...
        for i in range(5):  # 循环五次
            ...
            # self.yolo.detect(device='k4a', mode='detect', depth=False, rotate=1.0,range=0.5)
            self.detect()  # 检测垃圾
            self.catch()  # 抓取垃圾
            self.throw()  # 扔垃圾
    ...
    def detect(self):
        ...
        self.result = self.yolo.detect(device='k4a', mode='detect', depth=False, rotate=1.0,range=0.5)
        print(self.result)
        num = len(self.result)
